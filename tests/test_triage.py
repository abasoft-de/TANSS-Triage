...
Beschreibung: Prueft das robuste Parsen der LLM-Antwort und die Absicherung
...
def test_parst_json_in_codezaun():
...
def test_unbrauchbares_liefert_none():
...
def test_melder_id_muss_positive_zahl_sein():
    result = parse_triage_json(
...
def test_run_triage_verwirft_fremde_melder_id():
...
def test_run_triage_akzeptiert_bekannte_melder_id():
